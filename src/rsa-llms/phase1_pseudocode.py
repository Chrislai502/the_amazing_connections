...
def rsa_connections_solver(game: Connections):
    ...
    while level < 4:
        ...
        category_utterances = s1.choose_categories(curr_group.members, num_samples=1)
        print(f"Generated categories: {category_utterances}")

        # Pragmatic Listener (L1)
        guesses = l1.guess(category_utterances[0], num_samples=1) # we could try later on with multiple
